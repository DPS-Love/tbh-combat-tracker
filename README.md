...
另有一个仿 ACT 的战斗记录主面板：每一段战斗都能回看，每局的战斗事件自动存成日志，可以导入重新解析。
面板文本跟随游戏语言。
...
- 浮窗标题栏按钮在 **输出 / 承伤 / 治疗** 三个视图间切换；整个浮窗可拖动
...
主面板的「导入」可以打开任意一份日志，**按当前版本重新解析**——以后更新加了新的统计维度，旧日志导入也能看到。
...
| `HistorySize` | 200 | 主面板里本局保留多少段，更早的仍在日志里；`0` 为不限（最多 1000） |
...
| `LogEvents` | true | 写战斗日志；关掉的话主面板只有本局的数据 |
